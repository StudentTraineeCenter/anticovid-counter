...
bool AzureHelper::sendMessagePhoto(CameraHelper *camera, const std::map<String, String> &jsonData) {
    if (!isBusy && isAfterInterval()) {
        ...
        int jsonDocumentSize = 120000*sizeof(char); // 252 kB
        ...
        const char *s = camera->getPhotoGrayscaleString();
        doc["photo"]["data"] = s; //must be last
...
/*        logd(String(static_cast<long>(doc.size())));
        ...
        //auto *msg = (String*) ps_malloc(254000*sizeof(char));
        String msg ="";
        serializeJson(doc, msg);
        serializeJson(doc, Serial);

        EVENT_INSTANCE *message = Esp32MQTTClient_Event_Generate(msg.c_str(), MESSAGE);
        Serial.println(sizeof(message->stateString));
